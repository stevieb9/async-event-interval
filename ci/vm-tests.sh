#!/bin/sh
# Run Async::Event::Interval tests across multiple VMs and summarize results.
#
# Usage: ./ci/vm-tests.sh [-f] [-l] [-o] [-s] [-a] [-k] [-D] [-h] [prove options]
#
...
Examples:
  ...
  $(basename "$0") -f -l t/15-interval.t        # FreeBSD + Linux, single test
...
run_vm() {
    ...
    if [ $DISPLAY_MODE -eq 1 ]; then
        if "${SCRIPT_DIR}/${_script}" ${PROVE_ARGS}; then
