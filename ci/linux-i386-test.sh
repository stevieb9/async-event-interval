#!/bin/sh
# Run Async::Event::Interval tests in a 32-bit i386 Linux environment via
# Lima/QEMU.
#
...
Examples:
  ...
  $(basename "$0") -v t/15-interval.t    # verbose, single file
...
limactl shell "$VM" -- sh -lc "
    ...
    # has the Async::Event::Interval test deps even when carried over from a
    # previous project (e.g. ipc-shareable, which did not install
    # Parallel::ForkManager).
    sudo systemd-nspawn -D '${CHROOT}' cpanm --notest --quiet \\
        Test::SharedFork Mock::Sub Parallel::ForkManager
    # IPC::Shareable: --reinstall to always pull the latest from CPAN.
    # Async::Event::Interval requires >= 1.14 and there is no Debian pkg
    # we can trust to be current.
    sudo systemd-nspawn -D '${CHROOT}' cpanm --reinstall --notest --quiet \\
        IPC::Shareable
"
...
COPYFILE_DISABLE=1 tar -C "$(dirname "$HOST_REPO")" -czf - "$(basename "$HOST_REPO")" | \
    limactl shell "$VM" -- sudo tar -C "${CHROOT}/opt" -xzf - \
        --transform "s|^$(basename "$HOST_REPO")|async-event-interval|"
...
limactl shell "$VM" -- sh -lc "
    sudo systemd-nspawn -D '${CHROOT}' \\
        sh -c 'cd /opt/async-event-interval && PERL5LIB=lib prove -l ${PROVE_ARGS}'" \
    || _test_rc=$?

echo "==> Async::Event::Interval version tested..."
limactl shell "$VM" -- sh -lc "
    sudo systemd-nspawn -D '${CHROOT}' \\
        sh -c 'cd /opt/async-event-interval && perl -Ilib -MAsync::Event::Interval -e \"print qq(Async::Event::Interval \\\$Async::Event::Interval::VERSION\\n)\"'"
echo "==> IPC::Shareable version tested..."
limactl shell "$VM" -- sh -lc "
    sudo systemd-nspawn -D '${CHROOT}' \\
        sh -c 'cd /opt/async-event-interval && perl -Ilib -MIPC::Shareable -e \"print qq(IPC::Shareable \\\$IPC::Shareable::VERSION\\n)\"'"
