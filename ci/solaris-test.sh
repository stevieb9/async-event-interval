#!/bin/sh
# Run Async::Event::Interval tests in a local OmniOS CE VM (Lima/QEMU).
#
...
_timeout_run 1200 limactl shell "$VM" -- sh -lc '
    ...
    # CPAN deps — use gmake so ExtUtils::MakeMaker gets GNU make.  IPC::Shareable
    # has XS, so the compiler discovery above matters.  cpanm exits 1 on build
    # failure; || true past it so a single CPAN hiccup doesn'"'"'t abort the
    # whole run (the test invocation will fail loudly if a dep is missing).
    sudo env MAKE=gmake cpanm --notest \
        Test::SharedFork Mock::Sub Parallel::ForkManager 2>&1 || true
    # IPC::Shareable: --reinstall to always pull the latest from CPAN.
    # Async::Event::Interval requires >= 1.14.
    sudo env MAKE=gmake cpanm --reinstall --notest IPC::Shareable 2>&1 || true
'
