...
#
# The method script mounts the cidata ISO (vioscsi CD-ROM at c1t0d0s0) to
# read the current instance-id on every boot.  Lima generates a new
# instance-id on each `limactl start`, so a static saved id would go stale.
# The marker is written to both /var/run (OmniOS native) and /run (where
# Lima's hostagent checks).

BOOT_DONE_METHOD_LINES = [
    "#!/bin/sh",
    'CIDATA_DEV="/dev/dsk/c1t0d0s0"',
    'MNT="/tmp/.lima-cidata"',
    'mkdir -p "$MNT" 2>/dev/null',
    'if mount -F hsfs -o ro "$CIDATA_DEV" "$MNT" 2>/dev/null; then',
    "    IID=$(awk '/^instance-id:/{print $2}' \"$MNT/meta-data\" 2>/dev/null)",
...
def main():
    ...
    run(sock, f'mkdir -p /run 2>/dev/null; echo "{iid}" > /run/lima-boot-done',
        delay=0.8)

    out = run(sock, "cat /run/lima-boot-done", delay=0.8)
    if iid in out:
        print("[first-boot] Boot-done marker confirmed (/run and /var/run).")
    else:
        print(f"[first-boot] WARNING: unexpected /run content: {out!r}")
